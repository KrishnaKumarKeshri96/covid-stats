...
const ChartWrapper = ({ data, primaryColor, secondaryColor, yKey }) => {
  const [selectedIndex, setSelectedIndex] = useState(9);
  const [pageNo, setPageNo] = useState(0);
  const formattedData = data
    .slice(pageNo * CHART_LENGTH, (pageNo + 1) * CHART_LENGTH)
    .reverse();
    ...
    <div className="chart-wrapper">
      <div className="chart-header">
        <div className="chart-summary-container">
          ...
          <div className="chart-type">{capitalizeFirst(yKey)}</div>
        </div>
