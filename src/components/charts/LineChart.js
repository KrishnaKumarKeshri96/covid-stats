...
const LineChart = ({
    ...
    return (
      <>
        {data.map((element, index) => {
          ...
          const y =
            chartHeight -
            (element[yKey] / maximumYFromData) * chartHeight +
            padding;
          return (
            <circle
              cx={x}
  ...
  const XAxis = () => {
    ...
    return <Axis points={`${padding / 4},${y} ${width - padding + 10},${y}`} />;
  };

  const YAxis = () => {
    const x = width - padding + 10;
    return <Axis points={`${x},${padding} ${x},${height - padding}`} />;
  };
  ...
  const LabelsXAxis = () => {
    ...
    return data.map((element, index) => {
      const x =
        (index / maximumXFromData) * chartWidth + padding / 2 - FONT_SIZE;
      return (
        <text
          key={index}
          x={x}
  ...
  const LabelsYAxis = () => {
    ...
    return new Array(PARTS + 1).fill(0).map((_, index) => {
      ...
      const yCoordinate =
        chartHeight - chartHeight * ratio + padding + (4 * FONT_SIZE) / 5;
      const value = parseFloat(maximumYFromData * (index / PARTS)).toFixed(
        precision
      );
      if (value !== "0") {
        return (
